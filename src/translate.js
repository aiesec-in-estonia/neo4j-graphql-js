...
export const getMutationRelationDirective = resolveInfo => {
  return resolveInfo.schema
    .getMutationType()
    .getFields()
    [resolveInfo.fieldName].astNode.directives.find(x => {
      return x.name.value === 'relationMutation';
    });
};
...
const constructRelationMutation = (
  relationType,
  fromType,
  toType,
  resolveInfo
  ...
  const fromParam = args[0].name.value;
  const toParam = args[1].name.value;
  if (isAddMutation(resolveInfo)) {
    return `MATCH (f:${fromType} {id: $${fromParam}})
    UNWIND $${toParam} as tid
    MATCH (f)-[r:${relationType}]->(t:${toType} {id: tid})
    DELETE r
    RETURN t`
  } else if (isRemoveMutation(resolveInfo)) {
    return `MATCH (f:${fromType} {id: $${fromParam}})
    UNWIND $${toParam} as tid
    MATCH (t:${toType} {id: tid})
    MERGE (f)-[r:${relationType}]->(t)
    RETURN t`
  } else {
    throw new Error("Autogenerated relation mutation resolver not implemented")
  }
}
...
const customRelationMutation = ({
  ...
  const constructedQuery = constructRelationMutation(relationType, fromType, toType, resolveInfo)
    ...
    if (fieldName === 'AND' || fieldName === 'OR') {
      ...
      if (isObjectType(innerSchemaType)) {
        ...
        if (isExistentialFilter(filterOperationType, filterValue)) {
          serializedFilterParam = true;
          filterMapValue = null;
        } else if (isNeo4jTypeInput(typeName)) {
...
const serializeNeo4jTypeParam = filterValue => {
  const isList = Array.isArray(filterValue);
  if (!isList) filterValue = [filterValue];
